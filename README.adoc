...
+
image::docs/images/reference_architecture.png[]

. Sepsis Detection Runtime process:
+
image::docs/images/runtime_process.png[]

. highmediummitigation Subprocess: 
+
image::docs/images/subprocess.png[]
...
The demo scenario starts by submitting an HTTP POST with a FHIR bundle (which includes a a FHIR R4 Patient) to the HAPI FHIR Server. 
...
== my.healthflow.io
This is HealthFlow's application .
