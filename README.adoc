...
= Sepsis Detection Demo - SmileCDR
:numbered:
...
NOTE:  This branch of the demo uses the SmileCDR FHIR server (rather than the community hapi-fhir-server).
...
image::docs/images/hapi_fhir_home.png[]S
...
The SMileCDR FHIR server is backed by a PostgreSQL database.
...
$ oc get secret/pull-secret \
            ...
            > /tmp/private-image-repo-pull-secret.json
-----

. Set variables specific to gitlab image repo:
+
-----
PRIVATE_IMAGE_REPO_ACCOUNT_NAME=<changeme>
PRIVATE_IMAGE_REPO_ACCOUNT_PASSWORD=<changeme>
-----

. Add credentials of private gitlab image repo account to pull secret:
+
-----
oc registry login --registry="registry.gitlab.com/jbride-rht" \
        --auth-basic="$PRIVATE_IMAGE_REPO_ACCOUNT_NAME:$PRIVATE_IMAGE_REPO_ACCOUNT_PASSWORD" \
        --to=/tmp/private-image-repo-pull-secret.json
-----
...
oc set data secret/pull-secret \
        -n openshift-config \
        --from-file=.dockerconfigjson=/tmp/private-image-repo-pull-secret.json
-----
...
oc get secret/pull-secret \
        -n openshift-config \
        --template='{{index .data ".dockerconfigjson" | base64decode}}' \
        | jq -r '.auths|to_entries | .[]|select(.key | contains("registry.gitlab.com"))'
-----

. Optional:  Copy _pull-secret_ to a different namespace:
+
-----
oc new-project user1-sepsisdetection
oc get secret pull-secret -n openshift-config -o yaml \
        | sed 's/namespace: .*/namespace: user1-sepsisdetection/' \
        | oc apply -f -
-----
...
RHSSO_URL=https://$(oc get route sso -n redhat-rhoam-rhsso --template='{{ .spec.host }}')/auth
REALM_ID=user1-sepsis
retrieve_token_url="$RHSSO_URL/realms/$REALM_ID/protocol/openid-connect/token"
FHIR_SERVER_URL=https://$(oc get route smilecdr-server -n user1-sepsisdetection --template='{{ .spec.host }}')
-----
