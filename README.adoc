...
+
NOTE:  A Size of _Training_ is sufficient to support the demo.
. Click: _Submit_.

. Expect to receive an intial two emails providing process within the first 20 minutes of ordering.
. Expect a third email about another 20 minutes after the second email.
+
This third email will provide details regarding the OpenShift environment that the demo will run on.
+
Log into the OpenShift Console using the details provided in the email.

. After the third email arrives, wait about another 20 minutes for the demo to fully provision.

.. If you've logged into the new OpenShift environment, you can monitor demo installation progress by executing the following:
+
-----
$ oc logs -f -c manager $( oc get pod -n ansible-system | grep "^ansible" | awk '{print $1}' ) -n ansible-system
----- 

.. Upon successful completion, a log statement should appear similar to the following: 
+
-----
